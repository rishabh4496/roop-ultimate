...
_FACE.extend([
    ...
    {"id": "gpen_ultimate", "label": "GPEN Ultimate", "source": "app/roop/processors/Enhance_GPENUltimate.py", "class": "Enhance_GPENUltimate", "model": "GPEN-BFR-512.onnx", "run": "Run", "initialize": "Initialize", "release": "Release", "backend": "onnxruntime + cpu anti-halo finish", "precision": "provider policy", "input": "512x512 forced ffhq_512 crop", "output": "512x512 / input-size resize", "batch": "1", "contexts": "1 or VRAM-admitted pool", "streams": "provider-managed", "cuda_graph": "no", "quality_guards": "non-finite + local min/max halo bound"},
    {"id": "restore_ultra", "label": "Restore Ultra", "source": "app/roop/processors/Enhance_RestoreUltra.py", "class": "Enhance_RestoreUltra", "model": "restoreformer_plus_plus.onnx", "run": "Run", "initialize": "Initialize", "release": "Release", "backend": "onnxruntime + cpu anti-halo finish", "precision": "provider policy", "input": "512x512 forced ffhq_512 crop", "output": "512x512 / input-size resize", "batch": "1", "contexts": "1 or VRAM-admitted pool", "streams": "provider-managed", "cuda_graph": "no", "quality_guards": "non-finite + local min/max halo bound"},
    ...
    {"id": "tile_diffusion", "label": "Tile Diffusion Synthesizer", "source": "app/roop/processors/Enhance_TileDiffusion.py", "class": "Enhance_TileDiffusion", "model": "latent diffusion / dynamic 512 tiles with 64px feather", "run": "Run", "initialize": "Initialize", "release": "Release", "backend": "torch / diffusers FP16", "precision": "FP16 / INT8", "input": "dynamic tiled 512x512", "output": "input-size / high-frequency residual injection", "batch": "1", "contexts": "1", "streams": "default torch", "cuda_graph": "no", "quality_guards": "non-finite + exposure gate + chrominance lock"},
])
