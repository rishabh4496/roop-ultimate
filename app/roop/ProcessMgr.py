...
from roop.procmgr_tracking import TrackingMixin, track_source_index
...
class ProcessMgr(BatchProcessingMixin, StabilizationSchedulingMixin, MaskingMixin, ColorTransferMixin, MergerMixin, PixelBoostMixin, TrackingMixin):
    ...
    def __init__(self, progress):
        ...
        self.mask_stabilizer = None   # smooths the mask edge (anti-flicker)
        ...
        # look settings by itself.
        self._target_appearance = TargetAppearanceStabilizer(
            enabled=bool(getattr(roop.globals,
                                 'target_conditioned_appearance', False)),
            alpha=float(getattr(roop.globals,
                                'target_conditioned_appearance_temporal_alpha',
                                0.30) or 0.30),
    ...
    def initialize(self, input_faces, target_faces, options):
        ...
        if getattr(options, 'stabilize_enhancer', False):
            ...
            self._enh_stab_factory = None
        if getattr(options, 'stabilize_mask', False):
            from roop.one_euro import MaskStabilizer
            _mst = getattr(options, 'stabilize_mask_strength', 0.5)
            # motion_beta=0 (unlike the enhancer's default 8.0): the parent
            # class raises its cutoff — i.e. backs off smoothing — in
            # proportion to how fast the kps CENTROID moves in full-frame
            # coordinates, which is right for enhancer texture (real motion
            # legitimately changes what the pixels should look like, so
            # blending through it causes ghosting) but wrong for the mask.
            # The mask lives in the ALIGNED crop, which already cancels head
            # translation/rotation/scale — its residual jitter is segmentation
            # edge noise (a hairline landing a pixel differently frame to
            # frame), essentially uncorrelated with how fast the head is
            # moving in the original frame. Measured on a kissing close-up
            # (continuous real head motion): motion_beta=8.0 (inherited
            # default) reduced frame-to-frame delta in the mask-edge region by
            # only 2.9%, because the stabilizer read the ordinary head motion
    ...
    def swap_faces(self, frame, temp_frame=None, stabilize=False, frame_idx=None):
        ...
        if self.options.swap_mode == "first":
            ...
            if self.options.swap_mode == "all":
                ...
                track_source_map = getattr(self, '_track_source_map', None) or {}
                ...
                for detected_index, face in enumerate(faces):
                    ...
                    # _build_temporal_faces) is stamped with the id of the
                    # track the whole-clip tracker already bound it to —
                    # velocity-predicted-bbox IoU + Re-ID over every frame it
                    # has ever appeared in, not just this one. Trust that
                    # directly instead of re-deriving the same association
                    # from scratch via nearest-centroid on this frame's static
                    # positions alone, which is what let two people
                    # standing/leaning close together have their entries
                    # crossed — the "whoever's blob happens to sit nearest
                    # wins" ambiguity is exactly what a real tracker with
                    # motion prediction and multi-frame continuity does not
                    # have to guess at. See interacting-faces-recall-fixes-
                    ...
                    if exact is not None or best_j >= 0:
                        ...
                        # Claim the entry/track even when its source is unresolved,
                        # so a second face this frame can't re-match the same
                        # track — but NOT when we vetoed on identity, because then
                        # this entry most likely belongs to one of the other faces.
                        if veto is None:
                            if exact is not None:
                                claimed_track_ids.add(tid)
                            else:
                                claimed.add(best_j)
                    ...
                    if src_index is not None:
                        ...
                        # report points at the wrong thing. Neither an exact nor a
                        # spatial match means the appearance gate (ROOP_TRACK_EMB_MAX)
                        # refused every track for this face, or the pre-pass recorded
                        # no track on this frame. A match (exact or spatial) that
                        # carries no usable source never passed the assignment gate —
                        # a different problem with a different knob.
                        if veto is None:
                            _audit_hit('no track entry matched' if exact is None and best_j < 0
    ...
    def process_face(self, face_index, target_face:Face, frame:Frame, plate:Frame=None, region=None):
        ...
        itself.
        """
        # Gap-filled (`_interpolated`) and coasted (`_coasted`) faces ARE swap
        # candidates. They exist to hold the swap across the frames the detector
        # misses -- a hand or a microphone crossing the face, motion blur, a
        # brief profile -- and are bounded by their own guards (a 10-frame
        # interpolation between two real observations; MAX_COAST_FRAMES,
        # MIN_HITS_TO_COAST, the on-screen and collision tests, the shot-cut
        # test in `_coast_track_gaps`).
        #
        # NEGATIVE RESULT, 2026-09-18 -> 2026-09-21: 3a4bb18 refused every such
        # face here (`is_synthetic_face` -> return frame). That turned the
        # anti-flicker machinery into a flicker source: on every detector miss
        # the face popped back to the original for the length of the gap and
        # re-appeared when detection resumed, and with `temporal_detection: true`
        # in the live config that is the common case, not the edge case. If a
        # coast is ever seen painting onto background again, tighten the coast
        # guards in roop/tracker.py; do not refuse the class.
        ...
        enhanced_frame = None
        # Mask output is 1 = restore target / 0 = generated face.  Keep the
        # observation so the later identity-detail pass cannot write source
        # marks through an occluder or a manually excluded region.
        _img_mask = None
            ...
            if p.type == 'swap':
              with _prof('swap'):
                ...
                if (_target_appearance is not None and enhanced_frame is not None):
                    ...
                    # UltraMax, after any template round-trip has returned to
                    # swap-crop space. Dark targets therefore cannot be lifted
                    # into a bright, unnaturally clean face by a GAN prior.
        ...
        try:
            if enhanced_frame is not None:
                enhanced_frame = self.apply_merger_post(
                    enhanced_frame, aligned_img, appearance=_target_appearance)
            else:
                fake_frame = self.apply_merger_post(
                    fake_frame, aligned_img, appearance=_target_appearance)
        except Exception as e:
        ...
        # Phase 9: source identity-specific detail. This is deliberately the
        # last crop-space operation: GPEN/UltraMax/other restorers, post-enhance
        # colour matching, merger sharpening/blur/grain, and temporal low-band
        # blending have all finished. The detail source is the persistent V2
        # consensus map, never aligned target texture. The final paste mask and
        # ownership region still remain downstream, while the mask observation
        ...
        if _ids > 0.0 and rotation_action is None:
            try:
                ...
                if _detail is not None:
                    ...
                    with _prof('identity_detail'):
                        _detail_target, _detail_metrics = restore_identity_detail(
                            _detail_target, _detail, target_face=target_face, matrix=M,
                            matrix_shape=(subsample_size, subsample_size), strength=_ids,
                            visibility_mask=_visible, target_template=swap_template,
                            ...
                            track_id=_temporal_tid, motion=_detail_motion,
        ...
        # every chain, and its mask composites the plate back over the object
        # (`keep_face * (1 - m_occlusion)`, see roop/occlusion_mask.py), so the
        # object survives and the visible skin is swapped -- the same contract
        # every mask engine has always had.
        #
        # NEGATIVE RESULT, 2026-09-19 -> 2026-09-21: `occlusion_state == 'partial'`
        # used to discard the whole generated face here (restore the landmark
        # hull from the plate). It was gated at 8% of the interior 2d106
        # landmarks reading HIGH in the occluder mask, i.e. six landmarks, and
        # the occluder's "not visible face" is also hair, eyebrows at the
        # hairline and any soft mask edge, so on the user's clip it fired on 14
        # of 24 single-face previews (start_react.js log, 2026-09-21) with the
        # identity matcher already satisfied. The verdict also flips frame to
        # frame at the mask boundary, so the face appeared and disappeared --
        # the reported "flicker" -- and any real hand or microphone turned the
        # swap off entirely instead of being masked. Moving the stamp after the
        # mask stabilizer (778a40e) did not change that. The stamp itself is
        # kept for diagnostics; nothing in the render path refuses on it.
