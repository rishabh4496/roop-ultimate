...
def extract_inspection_frames(video_path, out_dir, prefix, num_samples=5):
...
def main():
    ...
    for i, vpath in enumerate(videos, 1):
        ...
        targets, groups = auto_capture_targets(vpath, expect=1, log_prefix="[Expression]", strict=False)
        if targets is None:
        ...
        prefix = f"clip{i}"
        extracted = extract_inspection_frames(out_file, inspect_dir, prefix, num_samples=4)
    ...
    print("\n" + "=" * 90)
    print("EXPRESSION BENCHMARK PROCESSING DATA SUMMARY")
    print("=" * 90)
