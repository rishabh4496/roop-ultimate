...
def preview(payload: dict = Body(...)):
    ...
    try:
        ...
        roop_globals.light_harmonizer = bool(payload.get("light_harmonizer", getattr(roop_globals.CFG, "light_harmonizer", False)))
        roop_globals.light_harmonizer_key_intensity = float(payload.get("light_harmonizer_key_intensity", getattr(roop_globals.CFG, "light_harmonizer_key_intensity", 1.0)))
        roop_globals.light_harmonizer_ambient_bias = float(payload.get("light_harmonizer_ambient_bias", getattr(roop_globals.CFG, "light_harmonizer_ambient_bias", 0.0)))
        roop_globals.light_harmonizer_eye_specular = float(payload.get("light_harmonizer_eye_specular", getattr(roop_globals.CFG, "light_harmonizer_eye_specular", 0.8)))
        roop_globals.light_harmonizer_shadow_occlusion = float(payload.get("light_harmonizer_shadow_occlusion", getattr(roop_globals.CFG, "light_harmonizer_shadow_occlusion", 0.6)))
...
_routes_output.API_TEMP = API_TEMP
_routes_output._last_output = _last_output
...
# not register duplicate FastAPI paths.
list_output = _routes_output.list_output
delete_output = _routes_output.delete_output
reveal_output = _routes_output.reveal_output
_open_shared = _routes_output._open_shared
get_file = _routes_output.get_file
