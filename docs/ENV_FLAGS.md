...
| `ROOP_ALLOW_TRT_SMALL_GPU` | 0 (off) | Explicit opt-in for experimental TensorRT on sub-7GB GPUs (e.g. RTX 3060 6GB Laptop). When 0 (the default), devices with strictly less than 7.0 GB VRAM reject TensorRT admission and fall back to CUDA to prevent host RSS bloat and engine build thrashing. When 1, experimental TensorRT is admitted while session pools remain strictly at 0/0 (single context + GPU lock). |
...
| `ROOP_TRT_SHAPE_PROFILE` | 1 (on) | Give TensorRT an explicit min/opt/max optimization profile, derived per model from its own graph. Only models with a genuinely dynamic input get one — on the shipped weights that is `retinaface_r50` (`[b,3,h,w]`, band 320/**512**/1280) and nothing else: the live swapper, every restorer and `yoloface_8n` are static exports and are left alone, which also means their engine cache namespace is untouched and they are not forced into a cold rebuild. `0` disables it, which is both the escape hatch and what makes the feature A/B-able. |
| `ROOP_DETECTOR_OPENVINO` | unset (off) | Route the small per-face graphs (detector, landmarks/recognition, mask) to an OpenVINO device — `NPU`, `GPU.1`, or `auto` to take the best accelerator that is not the discrete card — leaving the dGPU to the swapper and restorer. The primary chain is kept *behind* the OpenVINO EP, so an op it cannot take still runs on CUDA/CPU in the same session. **Off by default and unmeasured:** neither validated machine (RTX 4070 / RTX 3060) has an ORT build with `OpenVINOExecutionProvider`, and detection output feeds every identity gate — a different kernel's rounding shows up as a different face being swapped, which the swap audit cannot see because it counts intent, not outcome. Says so once and stays on the primary provider when it cannot be satisfied. |
| `ROOP_DETECTOR_OPENVINO_PRECISION` | unset | Optional `precision` passed to the OpenVINO EP (e.g. `FP16`) when the above is active. |
...
| `ROOP_SELECTED_HOLD` | 0.95 | How far past **max face distance** a face may sit and still be swapped, *when the whole-clip pre-pass has already bound its track to that same person*. This is the anti-flicker hold in the default Selected-person path (no "Lock face identities" needed — the binding is computed on every run with `temporal_detection` on). A single frame's embedding is a noisy reading of a person: on the reported clip, 178 of 190 faces refused "over the identity threshold" in one 800-frame window sat on a track bound to the selected person, with their distances bunched against the gate (62.6% within 1.05x, 98.9% within 1.20x, median 1.04x) — i.e. the same face the run swapped a frame earlier and swaps again a frame later. Refusing them is the swap blinking off. Looser than the match gate for the same reason `ROOP_TRACK_VETO` is: the gate that *confirms* an identity and the gate that *gives one up* cannot be the same number, or the decision flips with the noise. The hold never outranks a confirmed per-frame match, never applies to a face another selected person fits better, and never applies to a contaminated crop. Reported as `swapped (track continuity)` in the SWAP AUDIT. `0` disables it, restoring the per-frame-only decision. |
